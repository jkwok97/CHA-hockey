...
@Component({
  selector: 'app-player-goal-leader-table',
  templateUrl: './player-goal-leader-table.component.html',
  styleUrls: ['./player-goal-leader-table.component.css']
})
export class PlayerGoalLeaderTableComponent implements OnInit, OnDestroy {
  ...
  columns = ['team_logo','player_name', 'goals'];
  ...
  getStats(seasonType: string) {
    this._playerStatsService.getGoalLeaders(this.currentSeason, seasonType).pipe(
      takeWhile(() => this._alive)
    ).subscribe((leaders) => {
