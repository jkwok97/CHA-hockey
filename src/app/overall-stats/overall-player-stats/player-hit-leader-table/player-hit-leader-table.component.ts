...
@Component({
  selector: 'app-player-hit-leader-table',
  templateUrl: './player-hit-leader-table.component.html',
  styleUrls: ['./player-hit-leader-table.component.css']
})
export class PlayerHitLeaderTableComponent implements OnInit, OnDestroy {
  ...
  columns = ['team_logo','player_name', 'hits'];
  ...
  getStats(seasonType: string) {
    this._playerStatsService.getHitsLeaders(this.currentSeason, seasonType).pipe(
      takeWhile(() => this._alive)
    ).subscribe((leaders) => {
