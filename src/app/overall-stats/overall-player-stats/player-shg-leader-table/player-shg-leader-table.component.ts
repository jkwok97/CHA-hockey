...
@Component({
  selector: 'app-player-shg-leader-table',
  templateUrl: './player-shg-leader-table.component.html',
  styleUrls: ['./player-shg-leader-table.component.css']
})
export class PlayerShgLeaderTableComponent implements OnInit, OnDestroy {
  ...
  columns = ['team_logo','player_name', 'sh_goals'];
  ...
  getStats(seasonType: string) {
    this._playerStatsService.getShGoalsLeaders(this.currentSeason, seasonType).pipe(
      takeWhile(() => this._alive)
    ).subscribe((leaders) => {
