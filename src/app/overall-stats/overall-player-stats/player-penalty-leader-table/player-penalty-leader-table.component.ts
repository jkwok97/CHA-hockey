...
@Component({
  selector: 'app-player-penalty-leader-table',
  templateUrl: './player-penalty-leader-table.component.html',
  styleUrls: ['./player-penalty-leader-table.component.css']
})
export class PlayerPenaltyLeaderTableComponent implements OnInit, OnDestroy {
  ...
  columns = ['team_logo','player_name', 'penalty_minutes'];
  ...
  getStats(seasonType: string) {
    this._playerStatsService.getPenaltyLeaders(this.currentSeason, seasonType).pipe(
      takeWhile(() => this._alive)
    ).subscribe((leaders) => {
