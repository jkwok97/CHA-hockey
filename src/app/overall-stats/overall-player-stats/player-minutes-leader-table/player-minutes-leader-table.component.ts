...
@Component({
  selector: 'app-player-minutes-leader-table',
  templateUrl: './player-minutes-leader-table.component.html',
  styleUrls: ['./player-minutes-leader-table.component.css']
})
export class PlayerMinutesLeaderTableComponent implements OnInit, OnDestroy {
  ...
  columns = ['team_logo','player_name', 'minutes_played'];
  ...
  getStats(seasonType: string) {
    this._playerStatsService.getMinutesLeaders(this.currentSeason, seasonType).pipe(
      takeWhile(() => this._alive)
    ).subscribe((leaders) => {
