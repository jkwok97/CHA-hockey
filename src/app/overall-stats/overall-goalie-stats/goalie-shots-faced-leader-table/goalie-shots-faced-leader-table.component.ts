...
@Component({
  selector: 'app-goalie-shots-faced-leader-table',
  templateUrl: './goalie-shots-faced-leader-table.component.html',
  styleUrls: ['./goalie-shots-faced-leader-table.component.css']
})
export class GoalieShotsFacedLeaderTableComponent implements OnInit, OnDestroy {
  ...
  columns = ['team_logo','player_name', 'games_played', 'shots_for'];
  ...
  getStats(seasonType: string) {
    this._goalieStatsService.getShotsFacedLeaders(this.currentSeason, seasonType).pipe(
      takeWhile(() => this._alive)
    ).subscribe((leaders) => {
