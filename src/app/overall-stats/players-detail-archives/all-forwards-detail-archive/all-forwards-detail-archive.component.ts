...
@Component({
  selector: 'app-all-forwards-detail-archive',
  templateUrl: './all-forwards-detail-archive.component.html',
  styleUrls: ['./all-forwards-detail-archive.component.css']
})
export class AllForwardsDetailArchiveComponent implements OnInit, OnDestroy {
  ...
  getStats(seasonType: string) {
    this._playerStatsService.getForwardStatsByType(seasonType).pipe(
  ...
  getAllTimeStats(seasonType: string) {
    this._playerStatsService.getForwardStatsByTypeSummed(seasonType).pipe(
      takeWhile(() => this._alive)
    ).subscribe((playerStats: PlayerStat[]) => {
      this.players = new MatTableDataSource<any[]>(playerStats as []);
      this.isLoading = false;
