...
@Component({
  selector: 'app-all-players-detail-archive',
  templateUrl: './all-players-detail-archive.component.html',
  styleUrls: ['./all-players-detail-archive.component.css']
})
export class AllPlayersDetailArchiveComponent implements OnInit, OnDestroy {
  ...
  getStats(seasonType: string) {
    this._playerStatsService.getPlayersStatsByType(seasonType).pipe(
  ...
  getAllTimeStats(seasonType: string) {
    this._playerStatsService.getPlayerStatsByTypeSummed(seasonType).pipe(
      takeWhile(() => this._alive)
    ).subscribe((playerStats: PlayerStat[]) => {
      this.players = new MatTableDataSource<any[]>(playerStats as []);
      this.isLoading = false;
