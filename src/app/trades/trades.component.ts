...
export class TradesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.isLoading = true;
    setTimeout(() => {
      this.yearSelect.value = "23";
      const start = this.years.find(
        (year) => year.query === this.yearSelect.value
      ).dateStart;
      const end = this.years.find(
        (year) => year.query === this.yearSelect.value
      ).dateEnd;
        ...
        (resp) => {
          setTimeout(() => {
            this.transactions = resp.map((item, index) => ({
              ...
              team_one_players: this.formatPlayers(
                item.team_one_players,
                item.team_one_firstnames,
                item.team_one_lastnames,
                item.team_one_nhlids
              ),
              team_two: this.getTeamInfo(item.team_two_id),
              team_two_picks: item.team_two_picks,
              team_two_players: this.formatPlayers(
                item.team_two_players,
                item.team_two_firstnames,
                item.team_two_lastname,
                item.team_two_nhlids
              ),
            }));
  ...
  toTradeYear(event) {
    ...
    if (this.yearSelect.value >= "21") {
      ...
      this.year = this.years.find(
        (year) => year.query === this.yearSelect.value
      );
      this.tradePage = this.sanitizer.bypassSecurityTrustResourceUrl(
        this.year.link
      );
    }
  }
