...
export class TeamCurrentSalaryComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.getTeamPlayerSalary(
      this._route.snapshot["_urlSegment"].segments[2].path,
      this.currentSeason
    );
    this.getTeamGoalieSalary(
      this._route.snapshot["_urlSegment"].segments[2].path,
      this.currentSeason
    );
    this.getProtectedPlayers(
      this._route.snapshot["_urlSegment"].segments[2].path,
      this.currentSeason
    );
    this.getProtectedGoalies(
      this._route.snapshot["_urlSegment"].segments[2].path,
      this.currentSeason
    );
  }
  ...
  getProtectedPlayers(id: number, season: string) {
    this._salaryService
      .getProtectedPlayersById(id, season)
      .pipe(takeWhile(() => this._alive))
      .subscribe(
        (salaries: PlayerSalary[]) => {
          if (salaries) {
  ...
  getProtectedGoalies(id: number, season: string) {
    this._salaryService
      .getProtectedGoaliesById(id, season)
      .pipe(takeWhile(() => this._alive))
      .subscribe(
        (salaries: PlayerSalary[]) => {
          if (salaries) {
