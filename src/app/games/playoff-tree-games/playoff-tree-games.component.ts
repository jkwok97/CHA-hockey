...
export class PlayoffTreeGamesComponent implements OnInit {
  ...
  constructor(
    ...
    this.playoffTeams$ = this.teamStatsService
    .getTeamStatsBySeasonByTypeByConference(this.currentSeasonService.currentSeason, 'Regular');
  }
  ...
  ngOnInit() {
    this.playoffTeams$
      ...
      .subscribe((teams) => {
        this.westernTeams = this.setConferenceStandings(teams.find((conference) => conference['name'] === 'Western').teams.slice(0, 8));
        this.easternTeams = this.setConferenceStandings(teams.find((conference) => conference['name'] === 'Eastern').teams.slice(0, 8));
      })
  }
  ...
  setConferenceStandings(teams) {
    ...
    return teams.sort((a,b) => {
      ...
      conferenceStanding: teams.findIndex((t) => t.shortname === team.shortname) + 1,
